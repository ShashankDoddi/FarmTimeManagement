    ...
    if (empty($username) || empty($password)) {
        ...
        if (!$admin) {
            ...
            $log = $conn->prepare("
                ...
                VALUES (?, 'LOGIN_FAILED', 'admin', ?, 'Wrong password', 'web')
            ");
            ...
            $log = $conn->prepare("
                INSERT INTO audit_logs (admin_id, action_type, target_table, target_id, reason, source_channel)
                VALUES (?, 'LOGIN', 'admin', ?, 'Admin logged in', 'web')
            ");
            $log->bind_param('ii', $admin['admin_id'], $admin['admin_id']);
            $log->execute();
            $log->close();
...
<head>
    ...
    <style>
        ...
        .form-group input:focus {
            border-color: #696c2b;
            box-shadow: 0 0 0 3px rgba(105,108,43,0.12);
        }
        .form-options {
            display: flex;
        ...
        <form method="POST" action="login.php">
            <div class="form-group">
                ...
                <input
                    ...
                    id="username"
                    name="username"
                    placeholder="admin@farmtime.com"
