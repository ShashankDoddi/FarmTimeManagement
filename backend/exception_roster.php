...
<head>
    ...
    <style>.field-error { display:none; color:#dc3545; font-size:0.78rem; margin-top:4px; }</style>
</head>
<body>
<div class="dashboard-wrapper">
    <aside class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a href="exception_roster.php" class="nav-link active"><i class="bi bi-speedometer2"></i> Dashboard</a>
            ...
            <div class="modal-body">
                <div class="form-row-2">
                    <div class="form-field">
                        ...
                        <span id="err_first" class="field-error"></span>
                    </div>
                    <div class="form-field">
                        <label>Last Name *</label>
                        <input type="text" name="last_name" class="form-input" placeholder="e.g. Miller" required>
                        <span id="err_last" class="field-error"></span>
                    </div>
                </div>
                <div class="form-row-2">
                    <div class="form-field">
                        <label>Staff ID *</label>
                        <input type="text" name="staff_number" class="form-input" value="<?= $nextNum ?>" required>
                        <span id="err_staffnum" class="field-error"></span>
                    </div>
                    <div class="form-field">
                        <label>Email</label>
                        <input type="email" name="contact_email" class="form-input" placeholder="name@farm.local">
                        <span id="err_email" class="field-error"></span>
                    </div>
                </div>
                <div class="form-row-2">
                    <div class="form-field">
                        ...
                        <span id="err_role" class="field-error"></span>
                    </div>
                    <div class="form-field">
                ...
                <div class="form-field">
                    ...
                    <span id="err_phone" class="field-error"></span>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-brand"><i class="bi bi-person-plus me-1"></i> Register Staff</button>
                </div>
            </div>
        </form>
    </div>
...
document.getElementById('staffModal').querySelector('form').addEventListener('submit', function(e) {
    clearErrors();
    let valid = true;
    ...
    const phone     = this.contact_number.value.trim();
    const staffNum  = this.staff_number.value.trim();
    const roleId    = this.role_id.value;

    // Name validation — only letters, spaces, hyphens, apostrophes
    const nameRegex = /^[a-zA-Z\s\-\']+$/;
    if (!firstName) {
        showError('err_first', 'First name is required.'); valid = false;
    } else if (!nameRegex.test(firstName)) {
        showError('err_first', 'First name can only contain letters.'); valid = false;
    }

    if (!lastName) {
        showError('err_last', 'Last name is required.'); valid = false;
    } else if (!nameRegex.test(lastName)) {
        showError('err_last', 'Last name can only contain letters.'); valid = false;
    }

    // Staff number
    if (!staffNum) {
        showError('err_staffnum', 'Staff ID is required.'); valid = false;
    }

    // Role
    if (!roleId) {
        showError('err_role', 'Please select a role.'); valid = false;
    }
    ...
    if (email) {
        ...
        if (!emailRegex.test(email)) {
            showError('err_email', 'Please enter a valid email address.'); valid = false;
        }
    }
    ...
    if (phone) {
        ...
        if (!phoneRegex.test(phone)) {
            showError('err_phone', 'Phone must contain only numbers (7-20 digits).'); valid = false;
        }
    }
